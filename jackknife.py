...
Opis działania:
Najpierw skrypt wczytuje wszystkie opcje wiersza poleceń i ustawiamy wartości zmiennych na podstawie
wykrytych opcji. Jeśli któryś z parametrów wiersza poleceń nie spełnia naszych kryteriów, drukujemy
instrukcję obsługi. W następnym bloku kodu próbujemy naśladować funkcje netcata dotyczące odczytywania
danych ze standardowego strumienia wejściowego i przesyłania ich przez sieć.
CTRL-D wyjście z nasłuchu i interaktywne nasłuchiwanie. W ostatniej części kodu wykrywamy, że mamy
utworzyć gniazdo nasłuchujące i przetwarzać dalsze polecenia (wysyłanie pliku, wykonanie polecenia,
uruchomienie wiersza poleceń).
Napierw tworzymy obiekt gniazda TCP i sprawdzamy czy otrzymaliśmy jakieś dane w standardowym strumieniu
wejściowym. Jeśli wszystko jest w porządku, przesyłamy dane do komputera docelowego i pobieramy dane
w odpowiedz, aż się wyczerpią. Wszystko z odbieraniem i wysyłaniem jest powtarzane do meomentu aż użytkownik
wyłączy skrypt.
...
def showHelp():
    ...
    print ("-u --upload=destination\t\t - gdy odbierze połączenie, wysyła plik i zapisuje go w [destination]\n")
    print ("Przykłady:")
...
def main():
    ...
    # Będziemy nasłuchiwać i ewentalnie coś wysyłać, wykonywać polecenia oraz włączać
